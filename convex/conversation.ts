...
export const getMyConversation = query({
    ...
    handler: async (ctx, args) => {
        ...
        if(!user) throw new ConvexError("User nt found")

        const conversation = await ctx.db.query("conversation").collect()

        const myConversation = conversation.filter((conversation) =>{
